...
| Goal checkpoint | Goal Autopilot checkpoint endpoint | 授权的阶段检查业务入口；当前 `audio_ref` 校验未完全接入 Media/AI Gateway 前，不能声明生产可信音频 checkpoint |
...
- Goal checkpoint 当前可以接收 `audio_ref` 字段，但更多作为 checkpoint 输入和脱敏哈希；在接入 Media/AI Gateway 的 ownership、状态和引用校验前，不得把它声明为可信音频 checkpoint evidence。
...
- Goal intake 与 checkpoint 当前可以接收 `audio_ref` 字段；可信音频诊断和 checkpoint 媒体校验必须继续受 XCB-001/XCB-002 约束。
...
| 治理场景 | 授权入口 | 业务语义 |
| --- | --- | --- |
| 产品范围与优先级 | Product Manager agent、`docs/product/development_status.md`、feature registry、stage docs | 决定当前做什么、延期什么、属于哪个 feature/stage/increment |
...
- 当前 workflow 已明确 Product Manager 是用户入口和产品对象 owner，Development Orchestrator 负责路由，issue-management 只做 tracking。
...
- 不得把 baseline、stage、feature、increment、change request 和 Product Base 混成同一个 source of truth。
...
| 模块 | 责任 | 不允许 |
| --- | --- | --- |
| Product Manager | 分类请求、维护 roadmap/development status、feature registry、stage scope、increment definition 和优先级 | 写详细实现、测试、API schema 或被 issue/PR 替代 |
...
| Development Orchestrator | 检查产品对象门禁并路由 specialist agents | 决定产品优先级或绕过 missing increment/AC/TC gate |
