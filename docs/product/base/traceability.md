...
## Traceability Ownership Rule（追溯归属规则）

- 本文件是 Product Base 完整跨级 join 的唯一维护位置。
- 新建或触碰的行使用稳定 `Traceability Row ID`，并可连接 User Story、Vertical Slice、FR、Spec、AC、TC、SWC/Contract、Code Evidence 和 Test Evidence。
- FR、Spec、AC、TC、SWC allocation 和报告只记录各自直接上游与必要 scope guard，不重复整条链路。
- 下方既有表格属于 legacy Product Base evidence；未触碰行不要求批量迁移。
...
Code Evidence 与 Test Evidence 字段只记录当前可核查的产品/代码/测试证据，不记录旧 feature 文档。
...
- Product Manager 不得仅凭口头说明把需求标记为完成；Codex Root/QA 必须在后续阶段用测试、报告或明确例外补齐证据。
...
- 若要把“字段不得为空”变成机器门禁，需要由 Codex Root 在后续执行阶段路由到 QA 或工程实现，新增脚本或 CI 检查。
...
| TC ID | Traceability Row ID | Increment ID | WP ID | Spec ID | AC ID | 测试层级 | 自动化状态 | 测试脚本路径 | 执行命令 | 结果状态 | 证据报告 | Gap / Exception |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TC-PB-FR010-001 | TR-PB-011 | N/A - Product Base | N/A - legacy baseline | Flow-010 | AC-011 | integration（集成） | automated（已自动化） | `backend/src/test/java/com/speakeasy/AuthControllerTest.java` | `cd backend && JAVA_HOME=/opt/homebrew/opt/openjdk@17 mvn -q -Dmaven.repo.local=.m2/repository -Dtest=AuthControllerTest test` | passed（通过） | `docs/reports/test_report.md#2026-06-10-product-base-profile-avatar-xcb-003-tests` | XCB-003 hidden `/user/me/avatar` path and missing server-side `avatar_ref` update（XCB-003 暗藏 `/user/me/avatar` 路径且缺少服务端 `avatar_ref` 更新） |
| TC-PB-FR010-002 | TR-PB-011 | N/A - Product Base | N/A - legacy baseline | Flow-010 | AC-011 | unit（单元） | automated（已自动化） | `test/application/session_profile_coordinator_test.dart`; `test/services/app_session_profile_avatar_sync_test.dart`; `test/services/api_client_contract_test.dart` | `flutter test test/application/session_profile_coordinator_test.dart test/services/app_session_profile_avatar_sync_test.dart test/services/api_client_contract_test.dart` | passed（通过） | `docs/reports/test_report.md#2026-06-10-product-base-profile-avatar-xcb-003-tests` | Flutter profile edit updated local avatar but did not sync `avatar_ref` through `/user/me`（Flutter 资料编辑已更新本地头像，但没有通过 `/user/me` 同步 `avatar_ref`） |
| TC-PB-FR010-003 | TR-PB-011 | N/A - Product Base | N/A - legacy baseline | Flow-010; XCB-003 | AC-011 | contract（契约） | automated（已自动化） | `docs/architecture/openapi/speakeasy-api.yaml`; `lib/generated/api/speakeasy_api.dart`; `scripts/check_openapi_dart_drift.py` | `npm run check:api-contract && npm run check:dart-client-drift` | passed（通过） | `docs/reports/test_report.md#2026-06-10-product-base-profile-avatar-xcb-003-tests` | OpenAPI/generated boundary drift could allow hidden Flutter API paths（OpenAPI/generated 边界漂移可能让隐藏 Flutter API path 进入实现） |

## Merge-Back Rule（回并规则）
后续 increment（增量）只有在实现、验收、追溯、测试和报告证据完整或例外已记录后，才能由 Product Manager 批准合并进 Product Base traceability（Product Base 追溯矩阵）。合并时必须保留关联 increment（增量）、关联 FR/AC、实现证据、测试证据、人工验收结果和非目标变化。
