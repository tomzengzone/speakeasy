...
## 状态
Draft（草案） - 当前文件是 Product Base 下 `Identity & Account Lifecycle` 稳定模块的需求草案。本文同时包含两类需求：当前后端代码已经实现且可用代码证据追溯的代码基线需求，以及真实短信 OTP 的目标态需求。目标态需求必须标记为 Proposed / 待实现，不得被计入已实现代码覆盖。
...
- 范围：把当前后端已实现的身份认证、账号创建或解析、会话、当前用户、退出登录、账号删除、删除审计和基础安全门禁行为反向固化为 Product Base 模块需求，并新增真实短信 OTP 目标态需求，作为后续 spec、AC、TC 和实现的上游输入。
- 当前交付：补齐 12 个子章节中的已实现需求 item，新增真实短信 OTP 目标态需求，并在 `traceability.md` 中区分已实现代码证据和目标需求待实现状态；下游 `spec.md` 已作为 Draft 规格生成，但 Product Base merge、AC、TC 和测试证据仍待后续补齐。
...
| --- | --- | --- |
| `access-onboarding` | Included partial（部分纳入） | 登录门禁、首次账号创建、当前用户状态和首评前后的访问门禁。 |
| `identity-account-lifecycle` | Draft（草案） | 身份认证、账号生命周期、会话、删除、风控、审计、隐私和生产发布门禁的稳定模块候选。 |
| `profile-membership` | Related（相关） | 只引用 profile/current user 和账号设置入口；不承载会员权益、支付或订阅业务。 |

## 假设
- 当前文件不声明商业化身份系统已完整可发布。
- 真实短信 OTP 已在本文形成目标态需求，并已进入 Draft `spec.md`；仍需要在后续增量中补齐 AC、TC、架构/API/领域契约、实现和测试证据。
- Apple、WeChat、邮箱等生产级身份入口需要在后续增量中重新形成需求、规格、验收和测试证据。
...
- **IDENTITY-TOKEN-001** 系统必须签发服务端 opaque access token 和 refresh token。
- **IDENTITY-TOKEN-002** 系统必须使用 `SecureRandom` 生成 token 原始字节。
- **IDENTITY-TOKEN-003** 系统必须只持久化 access token 和 refresh token 的 hash。
- **IDENTITY-TOKEN-004** access token 默认有效期必须为 30 分钟。
- **IDENTITY-TOKEN-005** refresh token 默认有效期必须为 30 天。
- **IDENTITY-TOKEN-006** 受保护请求必须从 `Authorization: Bearer` header 提取 access token。
- **IDENTITY-TOKEN-007** access token 必须匹配 active 且未过期的 session 才能认证通过。
- **IDENTITY-TOKEN-008** access token 认证必须要求关联用户为 `active` 状态。
- **IDENTITY-TOKEN-009** refresh token 必须匹配 active 且 refresh 未过期的 session 才能刷新。
- **IDENTITY-TOKEN-010** refresh 成功必须轮换同一 session 的 access token hash 和 refresh token hash。
- **IDENTITY-TOKEN-011** refresh token 为空、无效、过期或已被轮换后，系统必须返回未认证错误。
- **IDENTITY-TOKEN-012** 后端安全配置必须使用 stateless session 策略。

当前不归档为已实现：JWT claims、JWT 签名校验、JWKS、refresh token reuse 风险告警。

### IDENTITY-ME 当前用户与 profile gate state
- **IDENTITY-ME-001** `/user/me` 必须只允许已认证用户访问。
- **IDENTITY-ME-002** `/user/me` 必须返回当前用户 ID、display name、avatar ref、locale、account status 和 onboarding status。
- **IDENTITY-ME-003** `/user/me` 必须返回当前 profile 的 target level 和 daily minutes。
- **IDENTITY-ME-004** 当前用户访问必须要求 access token 对应 active 且未过期 session，并且关联用户为 `active` 状态。
- **IDENTITY-ME-005** 用户必须能更新 display name、avatar ref、target level、daily minutes、reminder enabled 和 reminder time。
- **IDENTITY-ME-006** 系统必须拒绝非内置头像引用作为 avatar ref。
- **IDENTITY-ME-007** 用户完成首评后，系统必须把 onboarding status 更新为 `complete`。
- **IDENTITY-ME-008** 首页摘要必须根据 onboarding status 输出下一步动作。

当前不归档为已实现：独立 profile gate 判定对象、复杂账号权限矩阵。

### IDENTITY-LINK 身份绑定与解绑
- **IDENTITY-LINK-001** 新账号创建时必须绑定一个初始登录身份。
- **IDENTITY-LINK-002** 新建登录身份必须初始化为 `active` 状态。
- **IDENTITY-LINK-003** 身份解析必须使用身份来源和身份 subject。

当前不归档为已实现：已登录用户绑定第二登录身份、解绑身份、至少保留一个可登录身份、身份绑定二次验证、身份查询时过滤 inactive auth identity。

### IDENTITY-LOGOUT 退出登录
- **IDENTITY-LOGOUT-001** 已认证用户调用 logout 时，系统必须撤销当前 session。
- **IDENTITY-LOGOUT-002** logout 找不到当前 session 时，系统必须返回未认证错误。
- **IDENTITY-LOGOUT-003** session 被撤销时，系统必须把 session 状态设为 `revoked`。
- **IDENTITY-LOGOUT-004** session 被撤销时，系统必须记录 revoked time。
- **IDENTITY-LOGOUT-005** 已撤销 session 不得继续通过 access token 认证。

当前不归档为已实现：用户主动退出全部设备接口。

### IDENTITY-DELETE 账号删除与生命周期状态
- **IDENTITY-DELETE-001** 已认证用户必须能发起当前账号删除请求。
- **IDENTITY-DELETE-002** 账号删除请求必须要求 idempotency key 长度在 8 到 128 个字符之间。
- **IDENTITY-DELETE-003** 同一用户使用同一 idempotency key 重复发起删除请求时，系统必须返回已有删除 job。
- **IDENTITY-DELETE-004** 系统必须只允许 `active` 或 `deletion_requested` 账号发起删除执行。
- **IDENTITY-DELETE-005** 删除执行必须撤销该用户所有 active session。
- **IDENTITY-DELETE-006** 删除执行必须调用 AI account deletion retention runner 清理该用户关联的 AI media、TTS cache ownership 和 provider metric 数据。
- **IDENTITY-DELETE-007** 删除完成后，系统必须把账号状态标记为 `deleted`。
- **IDENTITY-DELETE-008** 删除完成后，系统必须把 display name 改为 `Deleted User`，清空 avatar ref，并把 onboarding status 标记为 `deleted`。
- **IDENTITY-DELETE-009** 已认证用户必须能查询当前账号最新删除 job 状态。
- **IDENTITY-DELETE-010** 运维用户必须能重试 failed 删除 job。
- **IDENTITY-DELETE-011** 删除重试必须只允许 failed 删除 job 进入重试执行；completed job 必须返回已有结果，其他状态必须被拒绝。
- **IDENTITY-DELETE-012** 删除重试认证必须允许携带同一 idempotency key 的 `deleted` 或 `deletion_requested` 用户重放同一个 `DELETE /user/me` 请求。
- **IDENTITY-DELETE-013** 删除执行必须删除该用户的 auth identity 记录。
- **IDENTITY-DELETE-014** 删除执行必须删除该用户的 user profile 记录。
- **IDENTITY-DELETE-015** 删除执行必须删除该用户的 onboarding assessment、learning route 和 user scenario state 记录。
- **IDENTITY-DELETE-016** 删除执行必须删除该用户的 practice session、practice turn 和 session summary 记录。
- **IDENTITY-DELETE-017** 删除执行必须删除该用户的 training session、training turn、training recap、training planner decision、training evidence candidate 和 training metric event 记录。
- **IDENTITY-DELETE-018** 删除执行必须删除该用户的 purchase、subscription、entitlement snapshot、usage ledger、usage reservation 和关联 payment provider event 记录。
- **IDENTITY-DELETE-019** 删除执行必须删除该用户的 learning evidence、learning history、mastery、review、practice queue、favorite expression 和 saved expression 记录。
- **IDENTITY-DELETE-020** 删除执行必须删除该用户的 goal profile、diagnostic assessment、mastery initial state、backplan、daily plan、plan item、autopilot control、goal idempotency、control idempotency、recovery decision、mastery transition decision、notification outbox、planner replay audit、progress forecast 和 outcome checkpoint 记录。

当前不归档为已实现：异步队列化删除保证、外部可观察的删除中阶段承诺、可恢复窗口、近期认证、用户自助取消删除。

### IDENTITY-RISK 风控、限流与防滥用
- **IDENTITY-RISK-001** 无效 bearer token 访问受保护接口时，系统必须返回 JSON 格式的未认证错误。
- **IDENTITY-RISK-002** admin 接口必须只允许 ops bearer token 认证通过。
- **IDENTITY-RISK-003** ops bearer token 必须以 hash 形式比较。

当前不归档为已实现：登录限流、OTP 发送限流、验证码失败次数限制、IP 风控、设备风控、CAPTCHA、账号锁定。

### IDENTITY-AUDIT 审计、隐私与合规
- **IDENTITY-AUDIT-001** 写入 audit log 时，系统必须清洗 target ref、request id 和 details。
- **IDENTITY-AUDIT-002** audit redaction 必须识别 token、secret、signature、receipt、URL 等敏感 key 或 value。
- **IDENTITY-AUDIT-003** 账号删除完成时，系统必须写入删除完成 audit event。
- **IDENTITY-AUDIT-004** 账号删除失败时，系统必须写入删除失败 audit event。
- **IDENTITY-AUDIT-005** 账号删除重试请求必须写入删除重试 audit event。
- **IDENTITY-AUDIT-006** 共享 audit log 查询必须记录查询行为 audit event。
...
当前不归档为已实现：生产环境禁用 fake OTP、生产环境禁用未校验 provider token、identity provider 配置缺失时阻断发布、identity 专属 release gate。现有 `/admin/release-health` 只是通用 warning，不是 identity 专属生产阻断。
...
## 后续合并规则
后续补齐本模块 `acceptance.md`、测试用例和测试证据后，必须更新 `traceability.md` 的 Spec/AC/TC 映射，并在实现、验收、追溯、测试和报告证据完整或例外已记录后，再由 Product Manager 决定是否合并到 Product Base 总库。
