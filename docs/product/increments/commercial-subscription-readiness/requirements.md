...
系统必须对 AI、ASR、TTS、发音评分等高成本能力实施用量限制、速率限制、基础成本预算、滥用检测和审计。对象存储上传链路、持久化 TTS 缓存、真实 DashScope evidence、AI 成本看板和生产 AI 数据策略由 `commercial-ai-provider-hardening` 承接。
...
- Paid AI voice 发布前必须额外完成 `commercial-ai-provider-hardening` 的媒体、缓存、真实 provider、成本看板和数据策略门禁。
