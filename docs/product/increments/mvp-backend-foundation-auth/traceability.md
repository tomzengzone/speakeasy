...
## 状态
Validated - foundation/auth gap closure evidence passed on 2026-05-29。
...
| MVP-BE-TR-002 | MVP-SI-002 | `mvp-backend-foundation-auth` | MVP-BE-FR-002 auth/session/current user | MVP-BE-SPEC-002 | AC-MVP-BE-002 | OpenAPI auth/current-user paths; `docs/architecture/openapi/dart-client-drift-manifest.json` | `backend/src/main/java/com/speakeasy/api/AuthController.java`; `backend/src/main/java/com/speakeasy/identity/`; `backend/src/main/java/com/speakeasy/security/`; `backend/src/test/java/com/speakeasy/AuthSessionLifecycleTest.java`; `backend/src/test/java/com/speakeasy/AuthControllerTest.java`; `backend/src/test/java/com/speakeasy/AuthServiceTest.java`; `package.json` | TC-MVP-BE-004: `AuthControllerTest`, `AuthServiceTest`, command `JAVA_HOME=/opt/homebrew/opt/openjdk@17 mvn -q -Dmaven.repo.local=.m2/repository "-Dtest=FoundationMigrationTest,PostgresFoundationMigrationTest,FoundationResponseContractTest,FoundationErrorContractTest,AuthControllerTest,AuthServiceTest,AuthSessionLifecycleTest" test`, result passed 2026-05-29; TC-MVP-BE-005: `AuthSessionLifecycleTest`, same command, result passed; TC-MVP-BE-006: `npm run check:api-contract` and `flutter test test/services/auth_service_test.dart`, result passed 2026-05-29; evidence `docs/reports/test_report.md` | N/A - no release-scope artifact in this increment; quality evidence in `docs/reports/quality_report.md` | Closed | MVP-BE-GAP-002 closed |
...
| MVP-BE-GAP-002 | Auth partial exists, but Flutter API client drift, current user/profile shape, and production/provider boundary need closure. | MVP-BE-TR-002 | Backend + Frontend + QA | Closed 2026-05-29: TC-MVP-BE-004/005/006 passed and evidence recorded. |
