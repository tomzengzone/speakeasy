...
## 状态
In progress - TC-COM-AI-001 through TC-COM-AI-007 have local backend/evidence-gate coverage；TC-COM-AI-004 has controlled live LLM/TTS/ASR sanity but remains release-blocked until `DASHSCOPE_AI_SANDBOX_EVIDENCE_REF` and independent external review are supplied.
...
| AC-COM-AI-003 | COM-AI-TR-003 | TC-COM-AI-004 | controlled live sanity passed / strict external evidence blocked |
...
| TC-COM-AI-004 | COM-SI-015 | FR-COM-AI-003 | COM-AI-SPEC-003 | AC-COM-AI-003 | COM-AI-TR-003 | COM-AI-GAP-003 | manual + release gate | controlled-live evidence-prep + structural gate / external-dependency | `tests/commercial/ai_provider_sandbox_matrix.md`; `scripts/check_ai_provider_sandbox_evidence.py`; `scripts/run_dashscope_sandbox_matrix.py` | `python3 scripts/run_dashscope_sandbox_matrix.py`; `python3 scripts/check_ai_provider_sandbox_evidence.py`; strict: `python3 scripts/check_ai_provider_sandbox_evidence.py --strict-external` | controlled live evidence-prep passed and wrote `build/reports/dashscope-sandbox-20260602T223557Z-3359fcc82fafa457.json`; non-strict gate passed; strict gate fails until evidence ref supplied | `docs/reports/test_report.md#2026-06-03-p01-local-blocker-closure-and-commercial-external-gate-revalidation`; local artifact `build/reports/dashscope-sandbox-20260602T223557Z-3359fcc82fafa457.json` | DashScope Qwen LLM, Paraformer ASR, TTS plus local fallback/cache/reject/error guards | 真实 provider 证据记录 latency、error code、cost、format compatibility、fallback 和独立审查；本地脱敏 matrix 只准备 evidence，不替代外部证据包；缺少 `DASHSCOPE_AI_SANDBOX_EVIDENCE_REF` 时不得关闭 release gate。 |
...
- Implementation/QA status: TC-COM-AI-001 through TC-COM-AI-003 and TC-COM-AI-005 through TC-COM-AI-007 have local automated evidence；TC-COM-AI-004 has structural evidence gate plus a repeatable sanitized controlled-live evidence-prep report.
- External/manual exceptions: `TC-COM-AI-004` still requires the externally stored DashScope evidence package, `DASHSCOPE_AI_SANDBOX_EVIDENCE_REF` and independent review before release closure.
