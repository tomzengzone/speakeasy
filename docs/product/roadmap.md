...
- 官方场景资产：`job_interview`、`onboarding_introduction`；课程与场景等级统一使用 CEFR `A1/A2/B1/B2/C1/C2`，当前资产覆盖 `A2/B1/B2`，`A1/C1/C2` 允许为空，尚不等于完整六级内容覆盖。
...
- 扩展现有 CEFR `A2/B1/B2` 内容覆盖，并为当前允许为空的 `A1/C1/C2` 逐步建设内容，但不立即承诺完整六级全覆盖。
