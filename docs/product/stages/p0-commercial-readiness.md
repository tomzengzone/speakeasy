...
| COM-SI-013 | 对象存储上传链路：Flutter 录音上传后由后端生成可信 provider-accessible `audio_ref` | required before paid AI voice | `commercial-ai-provider-hardening` | Planned |
| COM-SI-014 | 持久化 TTS 媒体缓存：多实例、重启、对象存储/CDN 复用和删除策略 | required before paid AI scale | `commercial-ai-provider-hardening` | Planned |
| COM-SI-015 | 真实 DashScope LLM/ASR/TTS sandbox 或 controlled live evidence | required before paid AI voice | `commercial-ai-provider-hardening` | Planned / external |
| COM-SI-016 | AI 成本看板：按套餐、用户、provider、模型、cache hit 和状态统计成本与毛利风险 | required before paid AI scale | `commercial-ai-provider-hardening` | Planned |
| COM-SI-017 | 生产级 AI 数据策略：音频、转写、provider payload、TTS cache、日志、账号注销和保留删除证据 | required before paid AI voice | `commercial-ai-provider-hardening` | Planned |
