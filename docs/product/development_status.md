...
## 活动目标
以 Product Base 活需求库承载已接受稳定能力，当前 committed stage work 先推进 MVP 后端与数据库全量补齐：
- MVP 后端线：把 Product Base 已接受的登录、首评、官方场景、练习会话、AI/语音、推荐表达、学习记忆、会员边界和测试发布证据补齐为服务端/API/数据库事实。
- 价值体验线：P0.1“口语优先、文本兜底的 FSI 式表达自动化训练闭环”保持 planned，但实现不得绕过 MVP 后端关键缺口。
- 商业发布线：P0 商业化订阅上线准备保持 planned/blocked-by-readiness，不得与 MVP 后端补齐混成同一个实现切片。
...
- Active MVP backend stage：`docs/product/stages/mvp-backend-foundation.md`
...
- MVP backend stage 已成为当前 active committed stage work；它不改变 Product Base 用户范围，而是补齐后端/API/数据库、客户端集成、测试和发布证据。
...
| MVP 后端与数据库 | 已有 foundation/auth/commercial partial，Product Base 全量未闭环 | `backend/`, `docs/architecture/openapi/speakeasy-api.yaml` | 当前 committed stage work，见 `docs/product/stages/mvp-backend-foundation.md` |
...
| MVP 后端与数据库全量补齐 | 接受，作为当前实际开发优先级；先补齐 Product Base 后端/API/DB/测试证据 | MVP |
...
17. 2026-05-28 每个 MVP backend increment 已建立 definition、requirements、spec、acceptance 和 traceability，并以 `MVP-SI-* -> MVP-BE-FR-* -> MVP-BE-SPEC-* -> AC-MVP-BE-* -> MVP-BE-TR-*` 建立全链路追溯。
18. Development Orchestrator 下一步应优先路由 `mvp-backend-foundation-auth`，再按 onboarding/content、practice/AI、learning/memory、membership/boundary、client/QA/release 的顺序推进。
...
- 当前 MVP 后端只完成 foundation/auth/commercial partial，Product Base 全量后端/API/DB、Flutter client integration、测试和发布证据仍是当前最大开发阻塞。

## 状态口径
当前不是从零开发新 MVP，而是在已有语音场景训练基线上补齐服务端事实。当前开发口径是：先完成 MVP 后端与数据库全量补齐，让 Product Base 能力具备 API、DB、测试和发布证据；随后再推进 P0.1 训练体验升级和 P0 商业化订阅上线准备。
