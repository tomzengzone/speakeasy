...
状态：拟议；Domain Schema 基线已纳入，并扩展覆盖 P0、P0.1 和 P0.2。
...
| Entity | Owner | 关键字段 | 生命周期 / 不变量 | Persistence / migration implication | API boundary recommendation | Test impact | Traceability note |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Scenario | Content / Scenario domain | scenario_id, slug, title, summary, category, status | Scenario 是主题，不是单等级对象；`scenario_id` 是跨版本稳定、可扩展且符合 lowercase snake-case 格式的 Content identity；一个已发布主题可有零个或多个不同 CEFR 的 ScenarioLevel，缺少可见内容轨道时仍可形成真实空状态；当前真实官方场景库存只有 `job_interview` 和 `onboarding_introduction`，该当前库存事实不封闭 `ScenarioId` 值域，新增官方主题仍须来自 owning approved product/content source | 需要 `content_*` 组表或受审核 bundled asset 迁移路径；不得为填补空等级而伪造内容 | Scenario API 后续提供完整已发布可见主题集合和详情；`ScenarioId` 机器边界由 `API_CONTRACT` / `OPENAPI` 约束为 `1..80` 位并匹配 `^[a-z0-9]+(?:_[a-z0-9]+)*$`，不得以当前两项库存定义为 enum；格式合法不表示对应 Scenario 存在、已发布或对当前学习者可见 | 场景目录完整性、开放稳定 ID、空主题、搜索、筛选、真实场景边界测试 | Product Base FR-003；P0.1 P01-FR-001；VS-CONTENT-001-1；FR-CONTENT-001 |
...
归属增量：`p0-2-goal-diagnostic-foundation`、`p0-2-goal-backplan-memory-policy`、`p0-2-autopilot-progress-checkpoint`、`p0-2-followup-b-autopilot-control-planner-memory`、`p0-2-followup-c-checkpoint-forecast-surfaces`、`p0-2-followup-d-release-gate-hardening`。
...
| OutcomeCheckpoint | Checkpoint domain | checkpoint_id, goal_profile_id, checkpoint_type, cadence, result_status, confidence_band, summary, plan_update_signal, reason_code | 每周/双周复测更新 evidence、risk、forecast 和 plan stale/replan signal；S003 结果状态必须区分 `recorded`、`low_confidence`、`failed`、`skipped` 和 unsupported/unavailable 拒绝路径；低置信度、失败或跳过不得产生达成声明 | 需要 `goal_outcome_checkpoints`；checkpoint result 与 plan stale 关联；不保存 raw transcript/audio | `POST /goal-autopilot/checkpoints` | TC-P02-FUC-007..009 checkpoint-to-plan、stale/replan、no false completion and control/recovery compatibility tests | P02-SI-012, P02-SI-013; P02-FUC-FR-003 |
| CheckpointPlanUpdateSignal | Checkpoint + Planner boundary | signal_type, reason_code, source_checkpoint_id, rule_version, input_snapshot_hash, replay_audit_id | S003 checkpoint-to-plan update 的传输/审计边界；只有 accepted-confidence checkpoint 改变计划方向时发出 `checkpoint_replan`，low-confidence/failed/skipped 只更新风险或 limitation；paused、blocked、recovery-required 或 stale plan 不得静默推进下一步行动 | 不新增持久表；使用 `PlannerReplayAudit` 保存最小化 input/output snapshot hash 和 replay hash | `POST /goal-autopilot/checkpoints` response `plan_update_signal`; `GET /goal-autopilot/replay-audits` audit projection | TC-P02-FUC-007..009 signal fields, replay audit, paused/recovery compatibility tests | P02-SI-012, P02-SI-013; P02-FUC-FR-003 |
| GoalProgressProjection | Goal Autopilot surface projection | projection_id, projection_state, downgrade_reason, goal, next_action, progress, latest_checkpoint, surface_fragments, source_refs, rule_version, updated_at | S004 backend-owned 聚合视图；Home/Queue/Wiki 只能消费 projection 或同源后端片段，不得本地推断最终 goal state、goal completion、ETA precision、claim guard 或 surface eligibility；`ready`、`limited`、`unavailable`、`deleted`、`unsupported`、`low_confidence`、`stale_plan`、`control_blocked` 均必须显式返回 reason | 不新增持久表；从 GoalProfile、UserAutopilotControl、PlanItem、ProgressForecast、OutcomeCheckpoint 和 replay/audit refs 计算；删除或 unavailable 时返回安全降级而不是旧事实 | `GET /goal-autopilot/progress-projection` | TC-P02-FUC-010..012 backend projection, controller safe-field and OpenAPI/Dart drift tests | P02-SI-006; P02-FUC-FR-004 |
| GoalProgressSurfaceFragment | Goal Autopilot surface projection | surface, display_state, eligible, downgrade_reason, next_action_ref, forecast_ref, checkpoint_ref, safe_fields | Home、expression queue 和 personal Wiki 的 surface-specific 安全字段边界；字段列表必须由后端确定，且不得包含 raw diagnostic transcript/audio、raw checkpoint payload、sensitive target details 或 provider payload | 与 GoalProgressProjection 同步计算；无需独立 persistence | `GET /goal-autopilot/progress-projection` surface_fragments | TC-P02-FUC-010..012 safe fields and source ownership tests | P02-SI-006; P02-FUC-FR-004 |
...
| OutcomeCheckpoint | `scheduled -> submitted -> evaluated -> recorded/low_confidence/failed/skipped`; accepted-confidence result may enter `plan_update_emitted`; unsupported or unavailable task is rejected without official-score certification |
| CheckpointPlanUpdateSignal | `not_required -> emitted`; emitted signal must cite source checkpoint id, rule version and input snapshot hash or replay audit id |
...
归属增量：`docs/product/increments/mvp-backend-practice-ai/`。
...
## P0.2 Followup-E Speaking Diagnostic Production Domain Contract（P0.2 Followup-E 口语诊断生产领域契约）
...
归属增量：`docs/product/increments/p0-2-followup-e-speaking-diagnostic-production/`。Phase 2 contract status 为 drafted；在 acceptance、test cases、traceability、OpenAPI 和 generated client 工作完成前，implementation 保持 blocked。
...
Followup-E 将 `DiagnosticAssessment` 从仅支持文本兜底的 intake 扩展为 audio-first 的 speaking baseline。该领域必须区分可信音频证据与用户输入文本、provider candidates 与 accepted diagnostic facts，以及 raw audio/transcript 数据与最小化后的 training facts。
...
- `DiagnosticUploadSession` 只能由已认证 owner 创建；相同 idempotency 输入 replay 时返回同一个 pending/completed session。
- `DiagnosticAudioSample` 的 `audio_ref` 只能由 backend 在 upload validation 之后生成；Flutter/local paths 不能成为领域事实。
- `DiagnosticQualityGate` 在质量过低时阻断 high-confidence diagnosis；provider candidates 不能覆盖 quality failure。
- `SpeakingDiagnosticAssessment` 扩展既有 `DiagnosticAssessment`；diagnostic mode 和 confidence 会限制 planner、forecast 与 checkpoint 的声明边界。
- `SpeakingDiagnosticResult` 只包含 accepted、安全且 learner-actionable 的事实；raw transcript、audio 和 provider payload 不进入结果。
- `DiagnosticPrivacyState` 让 deletion/export 行为显式、可审计；raw audio 和 provider payload 不是长期 learning facts。
...
- Upload session 可从 `created` 进入 `uploading`、`completed`、`validated`、`expired`，也可从 `created/uploading` 进入 `cancelled`。
- Audio sample 可从 `pending_upload` 进入 `uploaded`、`quality_checking`、`accepted`，或进入 `quality_rejected` / `deleted`。
- Diagnostic assessment 可从 `not_started` 进入 `audio_partial/text_only/audio_full`，再进入 `accepted_low|accepted_medium|accepted_high`，并可标记 `recalibration_available`。
- Privacy state 从 `active` 进入 `delete_requested` 再到 `deleted_or_redacted`；export 只能读取当前 safe/minimized state。
...
- `audio_full`：三个必需的 diagnostic sample types 都被 accepted，且 quality gates 全部通过。任何更低门槛都需要未来已批准的 scope change。
- `audio_partial`：至少存在一个 trusted audio sample，但 task coverage 或 quality 仍不完整。
- `text_only`：不存在 trusted audio sample；只能使用 user-entered text。
- `text_only` 不得填充 pronunciation、intonation、speech-rate、pause timing 或 acoustic fluency 等 acoustic dimensions。
...
- 持久化 upload sessions、accepted sample metadata、安全 `audio_ref`、quality gate result、diagnostic mode、confidence band、safe source refs 和 accepted diagnostic facts。
- 不得把 raw provider payload、provider secret、完整 signed URL、未受限 raw transcript、local file path 或 UI-only copy 持久化为 diagnostic facts。
- Raw audio retention 只能短期存在，并由 `DiagnosticPrivacyState` 管理；长期 training facts 必须最小化并脱敏。
...
- `GoalProfile 1 -> many SpeakingDiagnosticAssessment` 按 goal revision 建立关联。
- `SpeakingDiagnosticAssessment 1 -> many DiagnosticAudioSample` 用于承载 audio-backed samples。
- `DiagnosticAudioSample 1 -> 1 DiagnosticQualityGate`。
- `SpeakingDiagnosticAssessment -> GoalBackplan/ProgressForecast/OutcomeCheckpoint` 只能通过 accepted facts 以及 confidence/mode constraints 传递影响。
...
API contract 必须暴露 upload create/complete、diagnostic submission/status/result 和 delete actions。实现前必须先更新 OpenAPI source-of-truth；本领域章节不定义 machine-readable request/response schema。
...
测试必须证明 `audio_ref` 由 backend 拥有、quality downgrade 生效、text-only 模式省略 acoustic fields、deletion/export 做到最小化、provider candidate 会被拒绝，并且下游 low-confidence constraints 得到执行。
