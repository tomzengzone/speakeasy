...
`VS-ACC-001`～`VS-ACC-013` 实现为四个顺序交付包，最终形成完整的手机号
短信验证码注册/登录主链路，并将虚拟运营商号段风险接入保持为独立交付边界。

## Success Criteria

- `VS-ACC-001`～`VS-ACC-013` 均且仅映射到一个 WP/PR 单元，没有遗漏或重叠。
...
- WP-1：`VS-ACC-001`～`VS-ACC-006`，手机号输入、发送资格、主动发送、倒计时和脱敏反馈。
- WP-2：`VS-ACC-007`～`VS-ACC-009`，验证码数字输入、6 位提交资格和移动端自动填充。
- WP-3：`VS-ACC-010`～`VS-ACC-012`，验证成功后的账号判定、自动注册或直接登录。
- WP-4：`VS-ACC-013`，虚拟运营商号段风险因子接入。
...
- `VS-ACC-001`～`VS-ACC-013` 当前为 `draft`；实施前必须确认其行为边界已达到可执行状态。
- WP-2 开始前确认 Android OTP API 与 iOS 短信建议能力是否同时纳入当前交付；若平台能力未就绪，须修订计划并重新审批，不在执行中静默拆包。
- WP-4 依赖既有 `US-110` 风控接口或可替代契约；本任务不自行定义风控决策。

## PR Sequence

- [PR-001](prs/PR-001.md) — WP-1 手机号输入与验证码发送（VS-ACC-001～006）
- [PR-002](prs/PR-002.md) — WP-2 验证码输入与自动填充（VS-ACC-007～009）
- [PR-003](prs/PR-003.md) — WP-3 自动注册或登录判定（VS-ACC-010～012）
- [PR-004](prs/PR-004.md) — WP-4 虚拟运营商号段风险接入（VS-ACC-013）
...
已依据 `docs/product/user_stories/user_story_CAP_ACC.md` 中的 `VS-ACC-001`～`VS-ACC-013`
