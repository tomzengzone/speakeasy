...
## Before Release
- [x] Release scope selects approved Story/VS and any applicable approved FR when present without treating planning metadata as product behavior.
- [x] Applicable FR-TC and affected Contract-TC have run before selected VS integration/E2E evidence.
- [x] Canonical derived traceability resolves the selected VS and every present FR/Contract/TC branch.
...
- [x] The historical `commercial-ai-provider-hardening` requirements/spec/acceptance/test_cases/traceability packet remains frozen as provenance; current release lineage resolves through approved Story/VS, approved FR when present, typed TC, and derived traceability.
