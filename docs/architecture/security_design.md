...
- `POST /media/audio/uploads` 和 `POST /media/audio/uploads/{media_id}/complete` 是 paid AI voice 的唯一生产录音入口；对象存储签名 URL 必须短期有效、绑定 owner/purpose/content_type/byte_size/duration/checksum，并在完成时重新校验。
