...
- `POST /media/audio/uploads` 和 `POST /media/audio/uploads/{media_id}/complete` 是 paid AI voice 的唯一生产录音入口；当前实现方案使用阿里云 OSS private bucket 或等价 storage adapter。对象存储签名 URL 必须短期有效、绑定 owner/purpose/content_type/byte_size/duration/checksum/canonical object_ref，并在完成时重新校验；客户端不得持有 OSS AccessKey、长期下载 URL 或 provider read URL。
- Usage/audit 只能记录 hash/media id 等脱敏 source ref，不得持久化完整签名音频 URL、provider secret 或完整敏感 transcript。
- TTS cache key 可记录 normalized text hash/model/voice/language，不得记录完整敏感文本作为 cache index 或日志字段；TTS cache object 必须能按 expiry、账号删除或 retention job 删除/失效。
- P0 paid AI voice 前必须把“后续对象存储上传流程”和“持久化 TTS cache”从 residual 变为可执行实现，并通过 `commercial-ai-provider-hardening` 的测试和独立审查。
- 成本看板只能使用 user hash、plan、provider family、model、capability、status、duration/token estimate、cache hit、estimated cost、budget bucket 和 margin risk，不得暴露原始用户内容。
- Provider sandbox evidence API 只能返回 evidence id、provider family、capability、latency/error/cost/format/fallback 摘要、review status 和 redacted evidence ref；不得返回真实 provider request/response payload 或 API key。
...
- 按 user、device、IP risk class、entitlement status 和 provider family 做 rate limit。
...
- AI/ASR/TTS/scoring 使用 quota reservation/commit/release。
- 检测 repeated invalid receipts、scripted login、excessive ASR/TTS calls 和 suspicious restore attempts。
- Paid release 必须具备 budget alerts 和 provider error-rate alerts。
...
- OpenTelemetry traces 应传播 request context，但不能传播 sensitive payload。
- Admin audit access 必须受 role-gated 控制，且访问本身也要记录日志。
...
- Production secrets 必须存在，test secrets 必须不存在。
- Payment products 必须匹配 store metadata 和 backend allowlist。
- Android Billing 必须已完整启用并测试，或在 UI 中明确禁用。
- Account deletion flow 必须包含 backend job、local cleanup 和 user feedback。
- Privacy declaration 必须覆盖 audio、transcripts、AI provider processing、purchases 和 account deletion。
- Rollback plan 必须能禁用 paid gates 或 AI-heavy features，且不破坏 entitlement state。
...
P0-COM-ARCH-001 security 结论：controls 已在 architecture level 定义。Provider sandbox evidence、store metadata、secrets、signing 和 symbols 仍属于 DevOps/release execution evidence，不属于 architecture evidence。

## P0-AI-ARCH-001 Security Contract

| Control | Required decision | Evidence path |
| --- | --- | --- |
| Media upload signing | Upload create/complete binds user, purpose, MIME, byte size, duration, checksum and expiry; media ref is server generated and hash-audited. | `POST /media/audio/uploads`, `POST /media/audio/uploads/{media_id}/complete`, TC-COM-AI-001, TC-COM-AI-002 |
| ASR ref resolution | `/ai/transcribe` resolves only validated backend media refs; local paths, unsigned URLs, expired refs and wrong-owner refs fail closed before provider call. | `TranscribeRequest.audio_ref`, MediaAsset lifecycle |
| Persistent TTS cache | `/ai/tts` may return `media_id`, `cache_status` and `cache_expires_at`; raw text is not used as a persisted key and cache objects are deletion-aware. | TtsCacheEntry, TC-COM-AI-003 |
| Provider evidence | Ops evidence endpoint is restricted by `opsBearerAuth` and returns redacted evidence refs and aggregate compatibility/cost results only. | `GET /admin/ai/provider-evidence`, TC-COM-AI-004 |
| Cost dashboard | Ops metrics endpoint is restricted by `opsBearerAuth` and uses user hash plus aggregate plan/provider/model/status/cache/cost fields only. | `GET /admin/ai/cost-metrics`, TC-COM-AI-005 |
| Retention execution | AI retention jobs expose aggregate deletion/redaction counts and retry/manual failure state; raw audio/transcript/provider payloads are never exposed. | `POST /admin/ai/retention-jobs`, `GET /admin/ai/retention-jobs/{job_id}`, TC-COM-AI-006, TC-COM-AI-007 |

P0-AI-ARCH-001 security result: architecture/API/security contracts are sufficient for backend implementation to start, but production readiness remains blocked until code, tests, DashScope live evidence, retention proof, quality review and release evidence are produced.

P0-AI-ARCH-001 security 结论：architecture/API/security contracts 已足够支持 backend implementation 启动，但在 code、tests、DashScope live evidence、retention proof、quality review 和 release evidence 产出前，production readiness 仍保持 blocked。
...
- 最终 deployment provider 和 secret manager 尚未确定。
- 最终 auth token lifetime 和 refresh model 尚未确定。
- audio/transcript 按 jurisdiction 的精确保留期限尚未确定；P0 owner 是 `commercial-ai-provider-hardening` / COM-SI-017。
- P0 是否需要 dedicated admin console，或是否可以保持 ops-only，仍需确认。
