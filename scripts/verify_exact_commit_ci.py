...
def authority_graph_digest(root: Path) -> str:
    """Read the canonical authority-graph digest from the candidate validator."""
    root = root.resolve()
    validator = root / "scripts" / "validate_story_slice_cutover.py"
    if not validator.is_file():
        raise VerificationError("candidate cutover validator is missing")
    try:
        result = subprocess.run(
        ...
        if result.returncode not in (0, 1):
            detail = result.stderr.strip() or result.stdout.strip() or f"exit {result.returncode}"
            raise VerificationError(f"candidate cutover validator failed: {detail}")
        payload = json.loads(result.stdout)
        digest = payload["metrics"]["authority_graph_digest"]
        if re.fullmatch(r"[0-9a-f]{64}", str(digest)) is None:
            raise ValueError("invalid authority graph digest")
        return str(digest)
