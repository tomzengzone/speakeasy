...
## 状态
External pending - 本矩阵定义 `TC-COM-AI-004` 必需证据；2026-06-03 `scripts/run_dashscope_sandbox_matrix.py` 已生成本地脱敏 controlled-live evidence-prep 报告，但 strict release 仍需外部 evidence ref 和独立审查。
...
| Evidence-prep runner | `python3 scripts/run_dashscope_sandbox_matrix.py` writes a sanitized local report under `build/reports/` without API keys, raw audio URLs, full transcripts or raw provider payloads. |
