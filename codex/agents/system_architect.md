...
8. Create or update SWC allocation before implementation planning. Start from `codex/templates/swc_allocation.template.md`. Map `Traceability Row ID -> Increment ID -> WP ID -> FR -> Spec -> AC -> SWC/API/domain/data/test impact`; complete Story/Slice join remains in owning traceability.
