...
- Product Manager provides the product classification, registry/stage check result, increment id, scope, non-goals, upstream evidence, and required downstream artifacts.
...
- For `whole-app` architecture, Product Manager must provide a full scope inventory covering current Product Base, current APP baseline, feature registry, active stages, planned increments, future-stage boundaries, non-goals, commercial release gates, and known technical constraints.
...
5. Decide whether the request is current-stage, later-stage, rejected, or needs clarification.
6. If execution should continue, create or update the increment definition before producing a PM execution brief.
7. Produce a PM execution brief with classification, stage, increment id, scope, non-goals, current evidence, missing artifacts, and the question Development Orchestrator must answer.
8. After Orchestrator returns findings, update development status and summarize the product decision, next step, owner, blocker, and risk to the user.
...
2. If `whole-app`, build the scope inventory from Product Base, current baseline, feature registry, roadmap, all active stages, all planned increments, future-stage boundaries, and explicit non-goals before routing to System Architect.
...
- Every planned increment must link to feature registry, active stage, requirements, spec, acceptance, implementation, or release evidence when those artifacts exist.
